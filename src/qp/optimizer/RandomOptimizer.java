...
public class RandomOptimizer {
	...
	public Operator getOptimizedPlan2() {
		...
		double STARTING_TEMPERATURE; // starting T is assume to be 2 time the cost of initial starting plan
		double CURRENT_TEMPERATURE; // reflect the current temperature 
		double FINAL_TEMPERATURE = 1; // an mincost is assumed to be reach when the temperature drop to less than 1
		int EQUILIBRIUM; // the number of iteration for each temperature until the algo is considered to reach an equilibrium state. 	
		/** alpha is used to decrement the temperature linearly,
		 * the value should be between 0.8 and 0.99
		 * the higher the alpha the longer it take for temperature to reach freezing point
		 */
		double ALPHA = 0.8;
		...
		for (int i=0; i<NUMITER; i++) {
			...
			/*Setting up all the Temperature variables*/
			STARTING_TEMPERATURE = initCost * 2;
			CURRENT_TEMPERATURE = STARTING_TEMPERATURE;
			EQUILIBRIUM = (numJoin - 1) * 2;
			...
			if (minNeighborCost < MINCOST) {
				...
				while(CURRENT_TEMPERATURE > FINAL_TEMPERATURE) {// will break once current temperature < final which mean freezing point
					System.out.println("---------------while " + STARTING_TEMPERATURE + "Degree--------");
					...
					/**
					 * In the loop, randomly choos a random
					 * and pick the minimum cost for the next step
					 * 
					 * In the event where the neighCost > currentCost
					 * the neighbour is only chosen with a probability of e^ -C/T 
					 * Where C is the change in cost
					...
					for (int j =0; j < EQUILIBRIUM; j++) {
						...
						pc = new PlanCost();
						int changeInCost = pc.getCost(neighbor) - pc.getCost(initPlanCopy);
						System.out.println(changeInCost);
						
						if (changeInCost < 0) {
							minNeighbor = neighbor; // a downward move is always taken
							minNeighborCost = pc.getCost(neighbor);
						}
						if (changeInCost > 0) {
							double probability = Math.random(); // the probability which upward move state is taken
							double actualProbability = Math.exp((-(double) changeInCost) / CURRENT_TEMPERATURE);
